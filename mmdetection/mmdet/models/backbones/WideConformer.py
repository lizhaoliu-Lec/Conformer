...
class FCUUp(nn.Module):
    ...
    def __init__(self, inplanes, outplanes, up_stride, act_layer=nn.ReLU,
        ...
        self.conv_project = nn.Conv2d(inplanes, outplanes, kernel_size=1, stride=1, padding=0)
        self.ln = nn.LayerNorm(inplanes, eps=1e-6, elementwise_affine=False)
...
class WideConformer(nn.Module):
    ...
    def __init__(self, patch_size=16, in_chans=3, num_classes=1000, base_channel=64, channel_ratio=4, num_med_block=0,
        ...
        if representation_size:
            self.pre_trans_logits = nn.Sequential(OrderedDict([
                ('fc', nn.Linear(embed_dim, embed_dim)),
                ('act', nn.Tanh())
            ]))
            self.pre_cnn_logits = nn.Sequential(OrderedDict([
                ('fc', nn.Linear(int(256 * channel_ratio), int(256 * channel_ratio))),
                ('act', nn.Tanh())
            ]))
        else:
            self.pre_trans_logits = nn.Identity()
            self.pre_cnn_logits = nn.Identity()
    ...
    def forward(self, x):
        ...
        for i in range(2, self.fin_stage):
            ...
            if torch.isnan(x).any() or torch.isinf(x).any():
                print("Nan/Inf in convolutional branch output", force=True)
